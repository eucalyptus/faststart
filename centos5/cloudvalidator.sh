...
count=`$EUCALYPTUS/usr/sbin/euca_conf --list-walruses|tail -n+2|wc -l`
if [ $count -eq "0" ]
then
...
count=`$EUCALYPTUS/usr/sbin/euca_conf --list-clusters|tail -n+2|wc -l`
if [ $count -eq "0" ]
then
...
count=`$EUCALYPTUS/usr/sbin/euca_conf --list-scs|tail -n+2|wc -l`
if [ $count -eq "0" ]
then
...
retries=0;
while [ `$EUCALYPTUS/usr/sbin/euca_conf --list-nodes|tail -n+2|wc -l` -eq "0" ]
do
...
$INSTALL_DIR/imageinstall.sh $INSTALL_DIR/euca-centos-5.5-x86_64-13Mar2011.tgz admin
